...
inline lua_kind lua_kind_at(lua_State* state, int index = -1)
{
  return static_cast<lua_kind>(lua_type(state, index));
}
}
...
public:
  ...
  stack_reference(lua_State* state, int index) noexcept: state(state),
...
public:
  lua_kind lua_kind() const noexcept
  {
    return lua_kind_at(state, index);
  }
  ...
  {
    return lua_kind() != lua_kind::nil && lua_kind() != lua_kind::none;
  }
...
public:
  ...
  registry_reference(lua_nil_t) noexcept = default;

private:
  lua_State* state_ = nullptr;

  int index_ = LUA_NOREF;
};
}
